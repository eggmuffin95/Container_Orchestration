...
locals {
  aws_efs_gp    = "${join("", aws_efs_file_system.efs.*.id)}"
}
...
cloudstor_plugin_options="CLOUD_PLATFORM=AWS EFS_ID_REGULAR=$${efs_gp} EFS_SUPPORTED=$${efs_supported} AWS_STACK_ID=$${stack_id} AWS_REGION=$${region}"

docker_storage_volume="/dev/xvdf"
EOF
...
data "template_file" "windows_worker_passwords" {
  ...
  template = "${rsadecrypt(element(aws_instance.ucp_worker_windows.*.password_data, count.index), file(var.key_path))}"
}
...
module "inventory" {
  ...
  docker_ucp_lb = "${var.docker_ucp_lb == "" ? aws_elb.ucp-elb.dns_name : var.docker_ucp_lb}"
  docker_dtr_lb = "${var.docker_dtr_lb == "" ? aws_elb.dtr-elb.dns_name : var.docker_dtr_lb}"
}
